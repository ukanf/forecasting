...
# 1) load dataset
# ***
data_file_name = 'black_hills2018_c.csv'
df = read_csv(data_file_name, engine='python')
...
dataplot1 = dataset1[look_back:,target_col]  #shift training data
...
if file_write == "y":
    ...
    str_lead = str(lead_time)

    # generate filename and remove extra periods
    filename = 'FinErr_lstm_'+ str(n_epochs) + str_lead + '_' + stamp + '.xlsx'    #example output file
...
plt.close('all')
plt.plot(testY[0:250,:])
plt.plot(testPredict[0:250,:])
plt.xlabel('Hours')
plt.ylabel('ln(X*10$^{14}$)')
plt.legend()
plt.show()
plt.savefig('Fig_' + str_lead + '.png', format = 'png', dpi = 1200)
